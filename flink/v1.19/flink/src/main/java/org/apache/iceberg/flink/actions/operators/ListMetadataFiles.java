...
public class ListMetadataFiles extends ProcessFunction<SerializableTable, String> {
  private static final Logger LOG = LoggerFactory.getLogger(ListMetadataFiles.class);

  private final String name;
  private transient Counter errorCounter;

  public ListMetadataFiles(String name) {
  ...
  @Override
  public void processElement(SerializableTable table, Context ctx, Collector<String> collector)
      throws Exception {
    try {
      table
          .snapshots()
          .forEach(
              s -> {
                // Manifest lists
                collector.collect(s.manifestListLocation());
                // Snapshot JSONs
                ...
                s.allManifests(table.io()).stream()
