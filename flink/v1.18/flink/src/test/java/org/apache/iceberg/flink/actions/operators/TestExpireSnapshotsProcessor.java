...
class TestExpireSnapshotsProcessor extends OperatorTestBase {
  ...
  void testExpire(boolean success) throws Exception {
    ...
    List<RunResponse> actual;
    Queue<StreamRecord<String>> deletes;
    try (OneInputStreamOperatorTestHarness<SerializableTable, RunResponse> testHarness =
        ProcessFunctionTestHarnesses.forProcessFunction(
            new ExpireSnapshotsProcessor(0, "DUMMY", tableLoader, 0L, 1, 10))) {
      testHarness.open();
    ...
    assertThat(actual.get(0).success()).isEqualTo(success);
