...
public class IcebergSink
  ...
  private IcebergSink(
    ...
    this.checkFiles = flinkWriteConf.checkFiles();
  ...
  public SinkWriter<RowData> createWriter(InitContext context) {
    ...
    if (checkFiles) {
      fileChecker =
          new FileChecker(table.io(), dataFileFormat, table.schema(), equalityFieldIds, upsertMode);
    }
  ...
  public static class Builder implements IcebergSinkBuilder<Builder> {
    ...
    /**
     * Configure a file check which re-reads files after writing them.
     *
     * @param enabled If true, enable file checking.
     * @return {@link IcebergSink.Builder} to connect the iceberg table.
     */
