...
public class CatalogUtil {
  ...
  /**
   *  Load encryption manager factory from catalog properties, if not configured,
   *  will load org.apache.iceberg.encryption.DefaultEncryptionManagerFactory
   * @param properties catalog properties
   * @return Encryption manager factory class.
   */
  public static EncryptionManagerFactory loadEncryptionManagerFactory(Map<String, String> properties) {
    String impl = PropertyUtil.propertyAsString(
            properties, CatalogProperties.ENCRYPTION_MANAGER_FACTORY_IMPL,
            DefaultEncryptionManagerFactory.class.getName());
    DynConstructors.Ctor<EncryptionManagerFactory> ctor;
    try {
      ctor = DynConstructors.builder(EncryptionManagerFactory.class).hiddenImpl(impl).buildChecked();
    } catch (NoSuchMethodException e) {
      throw new IllegalArgumentException(String.format(
              "Cannot initialize EncryptionManagerFactory, missing no-arg constructor: %s", impl), e);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(String.format(
              "Cannot initialize EncryptionManagerFactory, %s does not implement EncryptionManagerFactory.", impl), e);
    }
