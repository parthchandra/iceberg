...
public class MeteredTableOperations implements TableOperations, Metered {
  ...
  public MeteredTableOperations(
      MetricRegistry metricRegistry, TableOperationsMetricType type, TableOperations delegate) {
    this.type = type;
  ...
  public TableMetadata refresh() {
    try (Context ctx =
        metricRegistry.timer(MetricRegistry.name(type.prefix(), TABLE_REFRESH_TIME)).time()) {
  ...
  public void commit(TableMetadata base, TableMetadata metadata) {
    try (Context ctx =
        metricRegistry.timer(MetricRegistry.name(type.prefix(), TABLE_COMMIT_TIME)).time()) {
