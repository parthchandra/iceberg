...
public class CoreMetricsUtil {
  ...
  public static TableOperations wrapWithMeterIfConfigured(
      Configuration conf, TableOperationsMetricType type, TableOperations delegate) {
    return wrapWithMeterIfConfigured(
        conf, delegate, t -> new MeteredTableOperations(metricRegistry, type, t));
  }

  private static <T> T wrapWithMeterIfConfigured(
      Configuration conf, T delegate, Function<T, T> wrapperFunc) {
