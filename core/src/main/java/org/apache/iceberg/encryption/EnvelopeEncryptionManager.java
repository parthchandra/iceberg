...
public class EnvelopeEncryptionManager implements EncryptionManager {
   ...
   */
  public EnvelopeEncryptionManager(KmsClient kmsClient, Map<String, String> tableProperties, String tableKeyId) {
    // At this point, we have an encrypted table
    this.nativeFormatEncryption = PropertyUtil.propertyAsBoolean(tableProperties,
    ...
    this.dataEncryptionConfig = EnvelopeConfiguration.builder()
            .singleWrap(tableKeyId)
            .useAlgorithm(EncryptionAlgorithm.valueOf(dataEncryptionAlgorithm))
            .build();

    this.dataKeyLength = PropertyUtil.propertyAsInt(tableProperties, ENCRYPTION_DEK_LENGTH,
