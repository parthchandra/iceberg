...
import static org.apache.iceberg.TableProperties.ITEV0_ENCRYPTION_KMS_CLIENT_CUSTOM_PROPERTIES_PREFIX;
import static org.apache.iceberg.TableProperties.ITEV0_ENCRYPTION_TABLE_KEY;
...
public class DefaultEncryptionManagerFactory implements EncryptionManagerFactory {
  ...
  private static class KmsClientSupplier {
    ...
    KmsClient create(Map<String, String> tableProperties) {
      ...
      // allowed).
      String kmsImpl = catalogProperties.get(CatalogProperties.ITEV0_ENCRYPTION_KMS_CLIENT_IMPL);
      if (null == kmsImpl && !ignoreTableProperties) {
        kmsImpl = tableProperties.get(TableProperties.ITEV0_ENCRYPTION_KMS_CLIENT_IMPL);
      }
      ...
      Preconditions.checkArgument(
          ...
          "KMS Client implementation class is not set (via "
              ...
              + TableProperties.ITEV0_ENCRYPTION_KMS_CLIENT_IMPL
              + " table property). "
      ...
      if (!ignoreTableProperties) {
        for (Map.Entry<String, String> property : tableProperties.entrySet()) {
          if (property.getKey().startsWith(ITEV0_ENCRYPTION_KMS_CLIENT_CUSTOM_PROPERTIES_PREFIX)) {
  ...
  public void initialize(Map<String, String> catalogProperties) {
    ...
    ignoreTableProperties =
        PropertyUtil.propertyAsBoolean(
            catalogProperties,
            CatalogProperties.ITEV0_ENCRYPTION_IGNORE_TABLE_PROPS,
            CatalogProperties.ITEV0_ENCRYPTION_IGNORE_TABLE_PROPS_DEFAULT);
  ...
  private String getTableKeyId(Map<String, String> tableProperties) {
    ...
    if (null == keyId) {
      keyId = catalogPropertyMap.get(ITEV0_ENCRYPTION_TABLE_KEY);
    }
    if (null == keyId && !ignoreTableProperties) {
      keyId = tableProperties.get(ITEV0_ENCRYPTION_TABLE_KEY);
    }
