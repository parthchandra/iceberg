 ...
 * An extension of the LocalKeyWrapClient class should be used only in situations where in-server wrapping is not
...
public abstract class LocalKeyWrapClient implements KmsClient {
  ...
  private static class LocalKeyWrap {
    ...
    private static LocalKeyWrap parse(String wrappedKey) {
      JsonNode json;
      try {
        json = JsonUtil.mapper().readValue(wrappedKey.getBytes(StandardCharsets.UTF_8), JsonNode.class);
      } catch (IOException e) {
      ...
      if (!LOCAL_WRAP_TYPE1.equals(localWrappingType)) {
        throw new RuntimeException("Can't parse unsupported localWrappingType: " + localWrappingType);
      }
  ...
  public String wrapKey(ByteBuffer keyBuffer, String wrappingKeyId) {
    ...
    checkMasterKeyLength(masterKeyLastVersion.getKey().length, wrappingKeyId, masterKeyLastVersion.getVersion());
    Ciphers.AesGcmEncryptor keyEncryptor = new Ciphers.AesGcmEncryptor(masterKeyLastVersion.getKey());
    String encryptedEncodedKey = Base64.getEncoder().encodeToString(keyEncryptor.encrypt(key, null));
    try {
  ...
  private void checkMasterKeyLength(int keyLength, String keyID, String keyVersion) {
    Preconditions.checkArgument((16 == keyLength || 24 == keyLength || 32 == keyLength),
        "Got key with wrong length: " + keyLength +
        " (must be 16,24 or 32). Key ID: "  + keyID + ", version: " +  keyVersion);
  }
