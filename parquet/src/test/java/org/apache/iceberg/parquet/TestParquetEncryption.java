...
public class TestParquetEncryption {
  ...
  private void writeEncryptedFile() throws IOException {
    ...
    FileAppender<GenericData.Record> writer = Parquet.write(outputFile)
        .schema(schema)
        .build();
  ...
  public void testReadEncryptedFileWithoutKeys() throws IOException {
    ...
    TestHelpers.assertThrows("Decrypted without keys", ParquetCryptoRuntimeException.class,
  ...
  public void testReadEncryptedFile() throws IOException {
    ...
    try (CloseableIterator readRecords = Parquet.read(inputFile)
          .project(schema)
          .callInit()
          .build()
          .iterator()) {
