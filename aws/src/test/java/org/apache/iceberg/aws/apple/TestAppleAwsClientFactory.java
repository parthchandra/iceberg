...
public class TestAppleAwsClientFactory {
  ...
  public void testSerialization() {
    Map<String, String> expectedConfig = ImmutableMap.<String, String>builder()
        ...
        .put("fs.s3a.appleconnect.accountId", "treeid").build();

    AppleAwsClientFactory factory = new AppleAwsClientFactory();
    factory.initialize(expectedConfig);

    AppleAwsClientFactory serializedFactory = SerializationUtils.deserialize(SerializationUtils.serialize(factory));
  ...
  public void testLowerCasedParams() {
    ...
    Map<String, String> lowerCaseConfig = ImmutableMap.<String, String>builder()
        .put("fs.s3a.appleconnect.username", "JAppleseed")
        .put("fs.s3a.appleconnect.password", "tree")
        .put("fs.s3a.appleconnect.totpsecret", "topttree")
        .put("fs.s3a.appleconnect.deviceid", "treehouse")
    ...
    expectedConfig.forEach((k, v) -> Assert.assertEquals("Missing config key", v, factory.config().get(k)));
  }
}
