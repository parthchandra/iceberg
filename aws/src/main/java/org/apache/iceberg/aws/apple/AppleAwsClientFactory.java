 ...
 * provided by {@link STSAssumeRoleFromCredentialsCacheProvider}. Configured using Iceberg Catalog
...
public class AppleAwsClientFactory implements AwsClientFactory {
  ...
  public void initialize(Map<String, String> properties) {
    ...
    List<String> missingRequired =
        REQUIRED_KEYS.stream().filter(k -> newConfig.get(k) == null).collect(Collectors.toList());

    Preconditions.checkArgument(
        missingRequired.isEmpty(),
        String.format(
            "Cannot initialize AppleAwsClientFactory missing properties %s", missingRequired));
