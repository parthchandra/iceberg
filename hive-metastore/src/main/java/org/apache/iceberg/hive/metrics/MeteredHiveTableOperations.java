...
public class MeteredHiveTableOperations extends HiveTableOperations implements Metered {
  ...
  public MeteredHiveTableOperations(
  ...
  public TableMetadata refresh() {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_REFRESH_TIME)).time()) {
  ...
  public void commit(TableMetadata base, TableMetadata metadata) {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_COMMIT_TIME)).time()) {
  ...
  protected long acquireLock() throws UnknownHostException, TException, InterruptedException {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_ACQUIRE_LOCK_TIME)).time()) {
  ...
  protected LockResponse lock(LockRequest lockRequest) throws TException, InterruptedException {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_LOCK_TIME)).time()) {
  ...
  protected LockResponse checkLock(long checkId) throws TException, InterruptedException {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_CHECK_LOCK_TIME)).time()) {
  ...
  protected void unlock(Optional<Long> lockId) {
    try (Timer.Context ctx =
        metricRegistry.timer(MetricRegistry.name(HIVE_TABLE_UNLOCK_TIME)).time()) {
      super.unlock(lockId);
    }
  }
}
