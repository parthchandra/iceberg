        ...
        .run(
            client -> {
              ...
              String defaultCatalog =
                  MetaStoreUtils.getDefaultCatalog(new HiveConf(conf, CachedClientPool.class));

              // Tests may return a mock of `HiveMetaStoreClient`, which causes
              // `getInvocationHandler` stuck.
              ...
              if (hiveMajorVersion.equals("2") && Proxy.isProxyClass(client.getClass())) {
                try {
                  ...
                  handler.invoke(
                      client,
                      HiveMetaStoreClient.class.getDeclaredMethod(
                          "setCurrentCatalog", String.class),
                      new Object[] {this.catalog});
                } catch (NoSuchMethodException e) {
                  // If users specify a custom HMS catalog other than default one, we should throw
                  // an exception.
                  if (!defaultCatalog.equals(this.catalog)) {
                    throw new RuntimeMetaException(
                        e, "HiveMetaStoreClient instance doesn't support `setCurrentCatalog` API.");
                  }
                } catch (Throwable t) {
                  if (!defaultCatalog.equals(this.catalog)) {
                    throw new RuntimeMetaException(
