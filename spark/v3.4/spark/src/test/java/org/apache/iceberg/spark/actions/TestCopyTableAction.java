...
public class TestCopyTableAction extends SparkTestBase {
  ...
  public void testManifestRewriteAndIncrementalCopy() throws Exception {
    String stagingWithScheme = stagingWithScheme();
    CopyTable.Result result =
        actions()
            .copyTable(table)
            .rewriteLocationPrefix(tableLocation, newTableLocation())
    ...
    int addedManifestsCount = Iterables.size(rewriteManifestResult.addedManifests());

    // only move version v4, which is the version generated by rewrite manifest
    CopyTable.Result postRewriteResult =
        actions()
            .copyTable(table)
  ...
  public void testFullTableCopyWithDeletedVersionFiles() throws Exception {
    ...
    checkMetadataFileNum(
        totalIteration - missingVersionFile,
        totalIteration - expiredManifestListCount,
        totalIteration,
        result);
    checkDataFileNum(totalIteration - missingDataFile, result);
  }
  ...
  protected void checkMetadataFileNum(
    ...
    Assert.assertEquals(
        ...
        filesToMove.stream()
            .filter(
                f -> f.contains("optimized-m-") && f.endsWith(".avro") || f.endsWith("-m0.avro"))
            .count());
    Assert.assertEquals(
  ...
  private void testCopyV2TableSnapshotCases(
    ...
    // Copy only the data added by snapshot 1
    checkDataFileNum(1, result);
    ...
    // Every new snapshot always adds one data file.
