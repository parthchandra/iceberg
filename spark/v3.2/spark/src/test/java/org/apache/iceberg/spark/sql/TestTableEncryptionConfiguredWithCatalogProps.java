...
public class TestTableEncryptionConfiguredWithCatalogProps extends SparkTestBaseWithCatalog {
  ...
  private static Map<String, String> appendCatalogEncryptionConfigProperties(Map<String, String> props) {
    Map<String, String> newProps = Maps.newHashMap();
    newProps.putAll(props);
    newProps.put(CatalogProperties.ENCRYPTION_KMS_CLIENT_IMPL, "org.apache.iceberg.spark.sql.MockKMS");
    newProps.put(CatalogProperties.ENCRYPTION_MANAGER_FACTORY_IMPL, DefaultEncryptionManagerFactory.class.getName());
    return newProps;
  }
  ...
  public TestTableEncryptionConfiguredWithCatalogProps(String catalogName, String implementation,
  ...
  public void testDelete() {
    ...
    List<Object[]> expected = ImmutableList.of(
            row(1L, "a", 1.0F), row(3L, "c", Float.NaN));

    assertEquals("Should return all remaining rows", expected,
  ...
  public void testDeleteWithoutKeys() {
    ...
    AssertHelpers.assertThrows("Must fail to modify encrypted table without key",
            SparkException.class,
            () -> sql("DELETE FROM %s WHERE id == '2'", tableName));
  }
}
