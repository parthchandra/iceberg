...
public class TestIcebergMetricSource {
  ...
  public static void startBefore() {
    SparkConf conf =
        new SparkConf(false)
            .set(
                "spark.metrics.conf.*.source.iceberg.class",
                "org.apache.iceberg.spark.metrics.IcebergMetricSource")
            .set(
                "spark.metrics.conf.*.sink.mocksink.class",
                "org.apache.iceberg.spark.metrics.TestIcebergMetricSource$MockSink");

    Assert.assertNull(
        "MockSink should be null. MetricsSystem is not initialized yet.", MockSink.instance);
    metricsSystem = MetricsSystem.createMetricsSystem("default", conf);
    metricsSystem.start(true);
    Assert.assertNotNull(
        "MockSink should not be null. MetricsSystem is initialized.", MockSink.instance);

    List<Source> sources =
        JavaConverters.seqAsJavaListConverter(metricsSystem.getSourcesByName("iceberg")).asJava();
    Assert.assertEquals("Exactly one source should be there for iceberg", 1, sources.size());
    Source source = sources.get(0);
    Assert.assertTrue(
        "iceberg metric source should be an instance of IcebergMetricSource",
  ...
  public void testIfMetersArePropagated() {
    ...
    Timer internalTimer =
        CoreMetricsUtil.timer(HADOOP_METRIC_PREFIX, MeteredTableOperations.TABLE_COMMIT_TIME);

    // The new timer should be available in Spark's metrics source.
    Timer sourceTimer =
        findTimer(
            icebergMetricSource.metricRegistry(),
    ...
    Assert.assertEquals(
        "Internal and source timers should be the same", internalTimer, sourceTimer);

    // The new timer should be available in Spark's metrics sink also.
    Timer sinkTimer =
        findTimer(
            mockSink.metricRegistry,
            MetricRegistry.name(HADOOP_METRIC_PREFIX, MeteredTableOperations.TABLE_COMMIT_TIME));
  ...
  public void testChildRegistriesAreTrackedByParent() {
    ...
    try {
      findTimer(
          icebergMetricSource.metricRegistry(), MetricRegistry.name(HADOOP_METRIC_PREFIX, newKey));
  ...
  private Timer findTimer(MetricRegistry metricRegistry, String keyEndsWith) {
    return metricRegistry.getTimers().entrySet().stream()
