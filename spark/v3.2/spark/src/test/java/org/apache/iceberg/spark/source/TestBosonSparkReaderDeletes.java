...
public class TestBosonSparkReaderDeletes extends TestSparkReaderDeletes {
  ...
  public static final Schema SCHEMA_WITH_FIXEDTYPE = new Schema(
      Types.NestedField.required(1, "id", Types.IntegerType.get()),
      // BOSON doesn't support fixed type yet, read fixed type will fall back to regular vectorization
  ...
  private void initFixedTypeTable() throws IOException {
    ...
    this.fixedTypeRecords = Lists.newArrayList(
    ...
    DataFile dataFileForFixedType = FileHelpers
        .writeDataFile(fixedTypeTable, Files.localOutput(temp.newFile()), TestHelpers.Row.of(0), fixedTypeRecords);

    fixedTypeTable.newAppend()
        .appendFile(dataFileForFixedType)
        .commit();
  }
