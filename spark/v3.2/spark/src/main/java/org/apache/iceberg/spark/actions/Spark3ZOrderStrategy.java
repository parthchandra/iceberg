...
public class Spark3ZOrderStrategy extends Spark3SortStrategy {
  ...
  public Set<DataFile> rewriteFiles(List<FileScanTask> filesToRewrite) {
    ...
    if (requiresRepartition) {
      distribution = SparkDistributionAndOrderingUtil.buildRequiredDistribution(table(), DistributionMode.RANGE);
  ...
  static class SparkZOrder {
    ...
    static Column sortedLexicographically(Column column, DataType type) {
      if (type instanceof ByteType) {
        ...
        return getLexicalBytesUTF8Like(ZOrderByteUtils.STRING_KEY_LENGTH).apply(column.cast(DataTypes.BinaryType));
      } else if (type instanceof BinaryType) {
        return getLexicalBytesUTF8Like(ZOrderByteUtils.STRING_KEY_LENGTH).apply(column);
