...
public class BosonColumnReader implements VectorizedReader<BosonIcebergVector> {
  ...
  /**
   * This method is to initialized/reset the ColumnReader. This needs to be called for
   * each row group after readNextRowGroup, so a new dictionary encoding can be set for
   * each of the new row groups.
   */
  public void reset() {
    if (delegate != null) {
      delegate.close();
  ...
  public void setBatchSize(int batchSize) {
    Preconditions.checkArgument(!initialized, "'reset' shouldn't be called  before 'setBatchSize' is called");
