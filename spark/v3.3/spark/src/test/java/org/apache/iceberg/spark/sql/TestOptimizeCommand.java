...
public class TestOptimizeCommand extends SparkCatalogTestBase {
  ...
  public void testOptimizeSortCommandFilter() {
    ...
    rows.sort(Comparator.comparing(l -> (String) l[0]));
    ImmutableList<Object[]> expected =
        ImmutableList.of(row("ALL", 10, 2), row("PartitionData{c1=0}", 10, 2));
    assertEquals("Should have correct results", expected, rows);
  }
  ...
  public void testOptimizeBinpackPartitioned() {
    ...
    rows.sort(Comparator.comparing(l -> (String) l[0]));
    ImmutableList<Object[]> expected =
        ImmutableList.of(row("ALL", 10, 1), row("PartitionData{c1=3}", 10, 1));
    assertEquals("Should have correct results", expected, rows);
  }
  ...
  public void testOptimizeUnpartitionedWithMultipleShuffleTasksPerFile() {
    ...
    List<Object[]> rows =
        sql(
            "OPTIMIZE TABLE %s " + "ORDER BY c2, c3 " + "OPTIONS ('%s'='%d', '%s'='%d')",
            tableName,
            SparkSortStrategy.SHUFFLE_TASKS_PER_FILE,
            3,
            RewriteDataFiles.TARGET_FILE_SIZE_BYTES,
            averageFileSize(table) * 5);
  ...
  public void testOptimizeZOrderPartitionedTable() {
    ...
    List<Object[]> rows =
        sql("OPTIMIZE TABLE %s ZOrder BY (c3, c1) %s", tableName, defaultSortOptions());
  ...
  public void testOptimizeZOrderTable() {
    createTable();
    List<Object[]> rows =
        sql("OPTIMIZE %s ZORDER BY (c1, c2) %s ", tableName, defaultSortOptions());
    ImmutableList<Object[]> expected = ImmutableList.of(row("ALL", 20, 4));
    assertEquals("Should have correct results", expected, rows);
  }
  ...
  public void testInvalidOptions() {
    ...
    AssertHelpers.assertThrows(
        "Should throw a sensible exception with bad filter",
        AnalysisException.class,
        "Could not translate",
        () -> sql("OPTIMIZE %s WHERE c3 = 2 %s", tableName, defaultSortOptions()));

    AssertHelpers.assertThrows(
        "Should throw a sensible error with a bad SortOrder",
  ...
  private void createPartitionedTable() {
    sql("DROP TABLE IF EXISTS %s", tableName);
    sql(
        "CREATE TABLE %s (c1 int, c2 string, c3 string) using iceberg PARTITIONED BY (c1)",
        tableName);
    List<ThreeColumnRecord> records1 = Lists.newArrayList();
  ...
  private void createBucketedTable() {
    sql("DROP TABLE IF EXISTS %s", tableName);
    sql(
        "CREATE TABLE %s (c1 int, c2 string, c3 string) using iceberg PARTITIONED BY (bucket(4, c3))",
        tableName);
    List<ThreeColumnRecord> records1 = Lists.newArrayList();
  ...
  protected int averageFileSize(Table table) {
    return (int)
        Streams.stream(table.currentSnapshot().addedFiles().iterator())
  ...
  protected <T> void shouldHaveLastCommitSorted(Table table, String column) {
    List<Pair<Pair<T, T>, Pair<T, T>>> overlappingFiles = getOverlappingFiles(table, column);
                    ...
                    Pair.of(
                        javaClass.cast(
                            Conversions.fromByteBuffer(
                                field.type(), file.lowerBounds().get(columnId))),
                        javaClass.cast(
                            Conversions.fromByteBuffer(
                                field.type(), file.upperBounds().get(columnId)))))
            ...
            .filter(
                filePair -> {
                  ...
                  boolean boundsOverlap =
                      (comparator.compare(leftUpper, rightLower) > 0
                              && comparator.compare(leftUpper, rightUpper) < 0)
                          || (comparator.compare(leftLower, rightLower) > 0
                              && comparator.compare(leftLower, rightUpper) < 0);
