...
public class DeleteReachableFilesSparkAction
  ...
  private Map<String, String> encryptionManagerConfPropsFromSparkConf() {
    ...
    Arrays.stream(spark().sparkContext().getConf().getAll())
        .forEach(
            kv -> {
              if (kv._1().contains(CatalogProperties.ITEV0_ENCRYPTION_MANAGER_FACTORY_IMPL)) {
                props.put(CatalogProperties.ITEV0_ENCRYPTION_MANAGER_FACTORY_IMPL, kv._2());
              } else if (kv._1().contains(CatalogProperties.ITEV0_ENCRYPTION_KMS_CLIENT_IMPL)) {
                props.put(CatalogProperties.ITEV0_ENCRYPTION_KMS_CLIENT_IMPL, kv._2());
              }
            });
    return props;
  }
