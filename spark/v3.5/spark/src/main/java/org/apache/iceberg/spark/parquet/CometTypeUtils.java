...
/**
 * Utility class for converting between Parquet types and Comet types. Since this class is in the
 * Spark module, it can access Comet dependencies directly without reflection.
 */
public class CometTypeUtils {
  ...
  public static ColumnDescriptor buildColumnDescriptor(ParquetColumnSpec columnSpec) {
    String physicalType = columnSpec.getPhysicalType();
    PrimitiveType.PrimitiveTypeName primType =
        PrimitiveType.PrimitiveTypeName.valueOf(physicalType);

    int maxRepetitionLevel = columnSpec.getMaxRepetitionLevel();
    int maxDefinitionLevel = columnSpec.getMaxDefinitionLevel();
    ...
    String[] path = columnSpec.getPath();
    String name = path[path.length - 1];

    String logicalTypeName = columnSpec.getLogicalTypeName();
    Map<String, String> logicalTypeParams = columnSpec.getLogicalTypeParams();
    ...
    int fieldId = columnSpec.getFieldId();
    int typeLength = columnSpec.getTypeLength();
    ...
    if (primType == PrimitiveType.PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY) {
      ...
      primitiveType = Types.primitive(primType, repetition).as(logicalType).id(fieldId).named(name);
    }
