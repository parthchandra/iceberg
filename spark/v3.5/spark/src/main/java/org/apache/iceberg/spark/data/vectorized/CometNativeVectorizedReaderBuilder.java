...
class CometNativeVectorizedReaderBuilder extends TypeWithSchemaVisitor<VectorizedReader<?>> {
  ...
  public VectorizedReader<?> struct(
    ...
    if (expected != null) {
      // NativeColumnReader can handle struct types for top-level fields
      Types.NestedField field = icebergSchema.findField(groupType.getId().intValue());
      if (field != null) {
        return new CometNativeColumnReader();
      }
      throw new UnsupportedOperationException(
          "Vectorized reads are not supported yet for nested struct fields");
    }
    return null;
  }
  ...
  public VectorizedReader<?> list(
      Types.ListType expectedList, GroupType array, VectorizedReader<?> unused) {
    if (expectedList != null && array.getId() != null) {
      // NativeColumnReader can handle list types for top-level fields
      Types.NestedField field = icebergSchema.findField(array.getId().intValue());
      if (field != null) {
        return new CometNativeColumnReader();
      }
    }
  ...
  public VectorizedReader<?> primitive(
    ...
    // All primitive types are supported by NativeColumnReader
    return new CometNativeColumnReader();
  }
}
